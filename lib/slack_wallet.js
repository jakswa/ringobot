...
class SlackWallet {
  ...
  withdraw(amountInBits, destination) {
    return new Promise((resolve, reject) => {
      this.wallet.sendMoney({
        to: destination,
        amount: amountInBits * BIT_VALUE,
        currency: "BTC"
      }, (err, tx) => {
        if (err) {
          console.log("withdraw failed", amount, this.wallet.id, destWallet.wallet.id, err);
          reject(err);
        } else resolve(tx);
      })
    })
  ...
  static forUser(userId) {
    return new Promise((resolve, reject) => {
      if (!WALLETS[userId]) WALLETS[userId] = this.promiseFor(userId);
      WALLETS[userId].then(resolve).catch(reject);
    })
  }
